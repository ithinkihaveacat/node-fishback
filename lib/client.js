...
Client.prototype.request = function (serverRequest, serverResponse) {
    ...
    var options = {
        ...
        "method": serverRequest.method,
        "headers": serverRequest.headers // @TODO copy headers?
    };
    ...
    var clientRequest = this.http.request(options, function (clientResponse) {
        ...
        emit('newResponse', serverResponse); // Can modify status via statusCode
        serverResponse.emit('endHead');      // and headers via setHeader()
    });
