...
function Proxy(cache)
{
    ...
    this.server = http.createServer((function(req1, res1) {
        ...
        function respondFromObject(entry) {
            
            res1.writeHead(entry.statusCode, entry.headers);
            ...
            console.log(req1.method + " " + req1.url.substr(0, 80) + " (cached " + entry.statusCode + "; " + t() + " seconds)");
            
        }
        
        function respondFromEmitter(res) {
            
            res1.writeHead(res.statusCode, res.headers);
            ...
            res.on('end', function() {
                res1.end();
                console.log(req1.method + " " + req1.url.substr(0, 80) + " (fetched " + res.statusCode + "; " + t() + " seconds)");
            });
        ...
        this.cache.find(req1, (function(obj) {
            
            if (typeof obj == 'object') {
                respondFromObject(obj);
            } else if (onlyWantsCache(req1)) {
                respondFromObject({
                    statusCode: "504",
                    headers: { }
                });
                request.call(this, [ obj ]);
            } else {
                request.call(this, [ obj, respondFromEmitter ]);
            }
...
// response via object
// response via emitter
// request, takes array of callback that are passed res2 

// cacheHit = response via object
// cacheMiss = respond via emitter 
