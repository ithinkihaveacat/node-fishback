...
function timer(digits)
{
    ...
    return function() {
        ...
        return new Number((t1 - t0) / 1000).toFixed(digits || 3);
    }
}
...
Cache.prototype.find = function(req, cacheHit, cacheMiss)
{
    ...
    if ((req.method == 'GET') && this.data[req.url] && wantsCache(req)) {
        ...
        // or null.  (We actually want entry = this.data[req.url].first(...).)
...
function Proxy(cache)
{
    ...
    this.server = http.createServer((function(req1, res1) {
        ...
        var request = this.request(req1, t); // callback; when called, do real request

        var respondFromCache   = this.respondFromCache(res1, t);   // callback; when called pipe data through res1
        var respondFromEmitter = this.respondFromEmitter(res1, t); // callback; when called pipe data through res1
 ...
 * @param {function () {}} when called, emits time since created, in seconds
 * @return {function (res) {}} res is an object representing a cache entry
 */

Proxy.prototype.respondFromCache = function (res1, t) 
{
    return (function (res2) {
        ...
        res1.end();
        console.log(res1.method + " " + res1.url.substr(0, 80) + " (cached " + res2.statusCode + "; " + t() + " seconds)");
    }).bind(this);
}
...
Proxy.prototype.respondFromEmitter = function (res1, t)
{
    return (function (res2) {
        res1.writeHead(res2.statusCode, res2.headers);
        ...
        res2.on('end', (function() {
            res1.end();
            console.log(res1.method + " " + res1.url.substr(0, 80) + " (fetched " + res2.statusCode + "; " + t() + " seconds)");
        }));
    }).bind(this);
};
