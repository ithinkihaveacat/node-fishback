...
function Proxy(cache, client) {
    ...
    this.on('request', function (req1, res1) {
        ...
        // First callback in arr to return non-null gets passed to callback
        ...
        var cacheFind = cache.find.bind(cache, req1);
        var clientFind = client.find.bind(client, req1);

        if (req1.method === 'GET') {
            req1.on('end', function () {
                req1.complete = true;
                first([cacheFind, clientFind], function (res2) {
                    if (res2.headers["x-cache"] === "MISS") {
                        cache.add(res2); // only add if not coming from cache!
                    }
